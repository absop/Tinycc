...
typedef struct _cctoken {
    unsigned char str[3];
    unsigned char tok;
} CCToken;

/* WARNING: the content of this string encodes token numbers */
static const CCToken cctokens[] = {
    {"<=", TOK_LE},
    {">=", TOK_GE},
    {"!=", TOK_NE},
    {"&&", TOK_LAND},
...
copy_const:
        ...
        default:
            if (v < TOK_IDENT) {
                ...
                const CCToken *q = cctokens;
                while (q < cctokens + sizeof(cctokens) / sizeof(CCToken)) {
                    if (q->tok == v) {
                        *(int *)p = *(int *)q->str;
                        /* *(p + 2) = '\0'; */
...
{
    if (handle_stray_noerror()) tcc_error("stray '\\' in program");
}
...
{
    while (isidnum_table[ch - CH_EOF] & IS_SPC) cinp();
}
...
redo:
        if (c == sep) break;
            ...
            if (c == '\\') {
                ...
                if (c == '\n')
                    file->line_num++;
                else if (c == CH_EOF) goto unterminated_string;
                else if (str) {
                    cstr_ccat(str, '\\');
                    cstr_ccat(str, c);
                }
...
static int expr_preprocess(void)
{
    ...
    while (tok != TOK_LINEFEED && tok != TOK_EOF) {
        ...
        if (tok == TOK_DEFINED) {
            ...
            if (tcc_state->run_test) maybe_run_test(tcc_state);
            c = define_find(tok) != 0;
            if (t == '(') {
                next_nomacro();
                if (tok != ')') expect("')'");
...
read_name:
            ...
            *s1->include_stack_ptr = file;
            ...
            for (; i < n; ++i) {
                ...
                if (tcc_open(s1, buf1) < 0) continue;
...
redo_no_start:
    ...
    switch (c) {
        ...
        case '\\':
            ...
            if (c != CH_EOF) goto redo_no_start;
            {
                TCCState *s1 = tcc_state;
                if ((parse_flags & PARSE_FLAG_LINEFEED) &&
                    ...
                    s1->include_stack_ptr--;
                    p = file->buf_ptr;
                    if (p == file->buffer) tok_flags = TOK_FLAG_BOF | TOK_FLAG_BOL;
                    goto redo_no_start;
                }
            }
            break;
...
ST_FUNC void preprocess_start(TCCState *s1, int is_asm)
{
    ...
    if (is_asm) tcc_define_symbol(s1, "__ASSEMBLER__", NULL);
